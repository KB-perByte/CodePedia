...
int main(){
    ...
    std::vector<long> blackCell(k, 0);

    for(long p = 0; p < k; p++){
        long row, col; scanf("%ld %ld\n", &row, &col);
        blackCell[p] = m * (row - 1) + (col - 1);
    }

    sort(blackCell.begin(), blackCell.end());
    ...
    for(long p = 0; p < t; p++){
        ...
        while(index < k){
            if(blackCell[index] < cellNumber){++index;}
            else if(blackCell[index] >= cellNumber){break;}
        }

        if(blackCell[index] == cellNumber){puts("Waste");}
        else{
