...
def check(st, n):
    ...
    while j<=n:
        tmp = st[i:j].count('A')
        if tmp == 0 or tmp == 3 or tmp == 1 :
            if tmp == 3:
                nwStrads+='A' 
            elif tmp == 1 and len(st[i:j]) == 1 : #need to verify 
                nwStrads+=st[i:j]
            else:
                nwStrads+='B'
            flag = False
...
while(tt<testCase):
    ...
    flag = False
    if N == 3:
        if _shards.count('A') > 0:
            flag = True
    while len(_shards)>3:
        _shards, flag  = check(_shards, len(_shards))
